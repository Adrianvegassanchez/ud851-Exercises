...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickOpenAddressButton(View v) {
        // TODODONE (5) Store an address in a String
        String address = "1600 Amphitheatre Parkway, CA";
        // TODODONE (6) Use Uri.Builder with the appropriate scheme and query to form the Uri for the address
        Uri.Builder uriBuilder = new Uri.Builder();
        uriBuilder.scheme("geo")
                .path("0,0")
                .query(address);
        Uri adressUri = uriBuilder.build();
        // TODODONE (7) Replace the Toast with a call to showMap, passing in the Uri from the previous step
    ...
    // TODODONE (1) Create a method called showMap with a Uri as the single parameter
    void showMap(Uri uri){
        // Do steps 2 - 4 within the showMap method
        // TODODONE (2) Create an Intent with action type, Intent.ACTION_VIEW
        Intent intent = new Intent(Intent.ACTION_VIEW);
        // TODODONE (3) Set the data of the Intent to the Uri passed into this method
        intent.setData(uri);
        // TODODONE (4) Verify that this Intent can be launched and then call startActivity
        if(intent.resolveActivity(getPackageManager())!=null){
            startActivity(intent);
        }
    }
